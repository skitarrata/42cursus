...
export default function Pong({ inverted }: { inverted?: boolean }) {
  ...
  const drawTextMontserrat = async (
    text: string,
    x: number,
    y: number,
    color: string
  ) => {
    await document.fonts.load('20px Montserrat');
    ctx.fillStyle = color;
    ctx.textAlign = 'center';
  ...
  const defaultCanva = async () => {
    ...
    if (watchId) return;
    if (friend)
      drawTextMontserrat(`Waiting for your friend...`, 375, 20, 'white');
    else drawTextMontserrat('Searching for an opponent...', 375, 20, 'white');
  };
            ...
            {opponent ? (
              ...
              <>
                <img
                  ...
                  className="game--image game-opponent"
                />
                <p className="game-text">Searching..</p>
                <p className="game-text">
                  0 <small>pts</small>
                </p>
              </>
